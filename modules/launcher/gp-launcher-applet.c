  ...
  if (info == 1000)
    {
       gtk_selection_data_set (data,
                               ...
                               8,
                               (const guchar *) priv->location,
                               strlen (priv->location));
    }
  else if (info == 2000)
    {
      char **uris;
      ...
      uris[0] = g_filename_to_uri (priv->location, NULL, NULL);
      uris[1] = NULL;
...
static void
setup_drag_source (GpLauncherApplet *self)
{
  GpLauncherAppletPrivate *priv;
  ...
  gtk_drag_source_set (priv->button, modifiers, NULL, 0, actions);

  target_list = gtk_target_list_new (NULL, 0);

  target = gdk_atom_intern_static_string ("application/x-panel-icon-internal");
  gtk_target_list_add (target_list, target, 0, 1000);

  target = gdk_atom_intern_static_string ("text/uri-list");
  gtk_target_list_add (target_list, target, 0, 2000);

  gtk_drag_source_set_target_list (priv->button, target_list);
  gtk_target_list_unref (target_list);

  g_signal_connect (priv->button, "drag-begin",
                    G_CALLBACK (drag_begin_cb), self);

  g_signal_connect (priv->button, "drag-data-delete",
                    G_CALLBACK (drag_data_delete_cb), self);

  g_signal_connect (priv->button, "drag-data-get",
                    G_CALLBACK (drag_data_get_cb), self);

  g_signal_connect (priv->button, "drag-end",
                    G_CALLBACK (drag_end_cb), self);

  g_signal_connect (priv->button, "drag-failed",
             ...
             const char       *icon_name)
{
  ...
  gtk_drag_source_set_icon_gicon (priv->button, icon);
  ...
  if (g_variant_lookup (initial_settings, "location", "&s", &location))
  ...
  else
    {
      ...
      if (error != NULL)
        ...
        {
          g_settings_set_string (settings, "location", filename);
        }
