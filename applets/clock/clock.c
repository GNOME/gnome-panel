...
set_hour_format_cb (GtkWidget * w, gpointer data)
{
  ...
  cd = (ClockData *) gtk_object_get_user_data (GTK_OBJECT (w));
  cd->hourformat = (int) data;
}
...
clock_properties (AppletWidget * applet, gpointer data)
{
  ...
  twentyfourhour = gtk_radio_button_new_with_label (gtk_radio_button_group (GTK_RADIO_BUTTON (twelvehour)), "24 hour");
  ...
  switch (cd->hourformat)
    {
    case 0:
      gtk_toggle_button_set_state (GTK_TOGGLE_BUTTON (twelvehour), TRUE);
      break;

    case 1:
      gtk_toggle_button_set_state (GTK_TOGGLE_BUTTON (twentyfourhour), TRUE);
      break;
    }

  gnome_property_box_append_page (GNOME_PROPERTY_BOX (dialog), table, gtk_label_new (_ ("Clock")));
